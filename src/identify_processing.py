...
def call_identify(args,seg_name,feature_path,current_dir):
    ...
    for i in range(len(seg_name)-1):
        for j in range(i+1,len(seg_name)):
            ...
            para_model.ckpt = f"{current_dir}/trained.pt"
...
def prediction(data, siamese, input_size = 40, n_thres = 20, mask_strain=[]):
    testDS = myDS(data,input_size, n_thres,mask_strain)
    ...
    for idx, data in enumerate(test_dataloader, 0):
        ...
        pro.append(output.detach().numpy())
...
def identify(config):
    ...
    info = torch.load(ckpt_path,map_location=torch.device('cpu'))
    ...
    # first prediction
    mask_strains_l1 = []
    leaf11_result = prediction(inputs,siamese,network.input_size, n_thres, mask_strains_l1)
    # second prediction
    mask_strains_l1 = list(ref_leaves[leaf11_result['score'].values>thres_mask]) # leaf masking 1
    leaf12_result = prediction(inputs,siamese,network.input_size, n_thres, mask_strains_l1)
    """ leaves segment 2 """
    mode = 2
    inputs = feature_processing(leaf_feature,mode)
    # first prediction
    mask_strains_l2 = []
    leaf21_result = prediction(inputs,siamese,network.input_size, n_thres, mask_strains_l2)
    # second prediction
    mask_strains_l2 = list(ref_leaves[leaf21_result['score'].values>thres_mask]) # leaf masking 2
    leaf22_result = prediction(inputs,siamese,network.input_size, n_thres, mask_strains_l2)
    """ leaves: two pro """
    ...
    clade11_result = prediction(inputs,siamese,network.input_size, n_thres, mask_strains_c1)
    """ clade: segment 2 """
    mode = 2
    mask_strains_c2 = mask_strains_l
    inputs = feature_processing(clade_feature,mode)
    clade21_result = prediction(inputs,siamese,network.input_size, n_thres, mask_strains_c2)
    """ clade: two pro """
    ...
    leaf13_result = prediction(inputs,siamese,network.input_size, n_thres, mask_strains_l1)
    ...
    leaf23_result = prediction(inputs,siamese,network.input_size, n_thres, mask_strains_l2)
    ...
    clade12_result = prediction(inputs,siamese,network.input_size, n_thres, mask_strains_c1)
    clade11_result.loc[replace_index] = clade12_result.loc[replace_index] 
    # segment 2
    mode = 2
    inputs = feature_processing(clade_feature,mode)
    mask_strains_c1 = mask_strains_l + mask_strains_c
    clade22_result = prediction(inputs,siamese,network.input_size, n_thres, mask_strains_c2)
