...
def call_single(args,seg_name,tree_path):
    ...
    for i in range(len(seg_name)):
        ...
        para_single = SimpleNamespace(tree1=ref_tree,tree2=tree_file,output=out_pic)
...
def call_split(tree_file,ref_leaves,length_thres=0.001):
    # max_size=5000
    """ min_size: include it """
...
def feature_single(config):
    ...
    """ 1. basic: call split """
    split,length,split_size,name,all_info = call_split(tree2_file,ref_leaves)
...
def length_to_code(mat_length,thres=0.01,base=5):
...
def compare_clade(all_info1,all_info2,min_clade_size=5,max_clade_size=150):
    """ load clade """
    split1 = all_info1[0]
    size1 = all_info1[1]
    string1 = all_info1[-1]
    ...
    for i in range(len(size1)):
        ...
        for j in range(len(size2)):
            ...
            if jd>0.80 and jd>found_jd:#found_jd the best hit among j for i 
                found_jd = jd
    ...
    sp2_all = []
    final_info = []
    retain_clade = unique_clade(final_info0,share_all0,0.8)
...
def clade_all(config,ref_leaves,tree_file1,tree_file2,all_info1,all_info2,mat_length1,mat_length2,mat_rank1,mat_rank2):
    min_clade_size = 5 # minimum matched clade size
    # base = config.base
    # distant_thres = config.distant_thres
    """ 1. call matched clades """
    common_clade,info_clade,clade_string = compare_clade(all_info1,all_info2,min_clade_size) # filtering clade size = 5
    if len(info_clade) ==0:
        return [],[],[],[],[],[]
    """
    ...
    clade_lcode1 = length_to_code(clade_length1)#,distant_thres,base)
    clade_lcode2 = length_to_code(clade_length2)#,distant_thres,base)
    ...
    clade_lcode = [clade_lcode1,clade_lcode2]# length coding clade
    clade_length = [clade_length1,clade_length2]
    clade_diff = [clade_diff1+1,clade_diff2+1]
    clade_rank = [clade_rank1,clade_rank2]
    return clade_meta,clade_order,clade_lcode,clade_diff,clade_rank,clade_length
...
def feature_pair(config):
    ...
    #base = config.base # base value for long length coding 20
    ...
    clade_meta,clade_order,clade_lcode,clade_diff,clade_rank,clade_length = \
