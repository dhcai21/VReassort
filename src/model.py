...
class myDS(Dataset):
    def __init__(self, df, vec_size, config, mask_na=[]):
        # Assign vocabularies.
        ...
        self.size = vec_size
        self.n_thres = config.n_thres
        self.mask_na = mask_na
        self.r_thres = 15
        self.l_thres = config.theta
        self.base = config.B
...
class myDS_train(Dataset):
    def __init__(self, df, config, mask_na=[]):
        # Assign vocabularies.
        self.input1 = df['s1'].tolist()
        self.input2 = df['s2'].tolist()
        self.na1 = df['na1'].tolist()
    ...
    def __getitem__(self, idx):
        ...
        r2 = np.array([int(float(i)) for i in self.r2[idx].split(',')], dtype='float32')
        name = self.name[idx]
        label = np.float32(self.label[idx])
        """ length """
        num1 = len(r1)
        num2 = len(r2)
        """ size and length """
